...
# subnet 4개
#
# sub1 : 10.0.1.0/24, 가용 영역 a
# sub2 : 10.0.2.0/24, 가용 영역 b
# sub3 : 10.0.3.0/24, 가용 영역 c
# sub4 : 10.0.4.0/24, 가용 영역 d
#
# 인터넷 게이트 웨이
# 인터넷 게이트 웨이로 보내는 라우트 테이블
# 서브넷 1,2,3,4 모두 이 라우트 테이블을 매핑
#
# inbound, outbound 모두 모든 ip에 대해 모든 port를 허용하는 보안 테이블
#
# AmazonS3FullAccess, AmazonEC2RoleforSSM 역할 생성
#
# Iam 인스턴스 프로파일 생성
#
# nginx proxy manager, redis, mysql 이 설치된 EC2 생성. (aws 수업페이지 참조)
#
# 사용할 ami => amazon_linux
#
# 사용할 서브넷 2 or 4
#
# 인스턴스 타입 t3.micro
#
# 보안 그룹 연결, 역할 연결, 퍼블릭 IP 설정,
#
# 인스턴스 태그 설정
#
# 태그명 : dev-ec2-1
